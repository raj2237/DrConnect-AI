...
class PatientDetails(BaseModel):
    ...
    visit_id: int = Field(...)
    doctor_id: int = Field(...)
    first_name: str = Field(..., example="Krishna Barot")
    last_name: str = Field(..., example="Krishna Barot")
...
def run_medical_agents(report_text: str) -> str:
    ...
    print("entered run medical function")
...
async def analyze_medical_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        """
        print("----------------------DB STARTTT-------------------------")
        print("ai_diagnosid_condition: ",initial_analysis.get('diagnosed_condition', 'Not specified'))
        print("ai_detailed_analysis: ",initial_analysis.get('detailed_analysis', ''))
        print("-----------------------DB ENDDD------------------------")
        # print("reportttt textttt: ",report_text)
...
@app.post("/get-patient-details", summary="Health check", response_model=PatientDetails)
def get_client_details(patient: PatientDetails):
